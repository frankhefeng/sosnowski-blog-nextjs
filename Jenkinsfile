                ...
                stage('Website') {
                    ...
                    steps {
                        withAWS(credentials:'blog') {
                            ...
                            sh "INVALIDATION_ID=`aws cloudfront create-invalidation --distribution-id $BLOG_CLOUDFRONT_DISTRIBUTION_ID --paths '/*' | jq -r .Invalidation.Id` && aws cloudfront wait invalidation-completed --distribution-id $BLOG_CLOUDFRONT_DISTRIBUTION_ID --id $INVALIDATION_ID"
                        }
                    }
                }
